...
> [!IMPORTANT]
> Качать Nulltrap можно только из этого репозитория. Сайты, предлагающие
> «Nulltrap» где-то ещё, к нам отношения не имеют. Это предупреждение стоит
> здесь с первого дня, потому что каждому проекту в этой нише приходилось
> добавлять его задним числом — после того, как появлялись поддельные сайты
> с загрузками.

## Установка

Скачайте `Nulltrap.exe` со [страницы релизов](https://github.com/L0lopop/Nulltrap/releases)
и запустите. Ставить заранее ничего не нужно — среда .NET лежит внутри файла, —
и прав администратора он не просит: кладёт себя в профиль пользователя и оттуда
забирает запуски Roblox.

Убрать можно кнопкой **«Удалить Nulltrap»** в настройках лаунчера либо через
список установленных программ Windows. Он спросит, оставить ли настройки и
загрузки.
...
**Быть проверяемым.** Подписанные релизы и воспроизводимые сборки. Самая
большая нерешённая проблема в этой нише в том, что пользователь не может
отличить настоящую загрузку от вредоносной, и технически её пока не решает
никто.
...
<img src="assets/status.svg" alt="Фазы 0, 1, 2 и 3 готовы, следующая — фаза 4" width="820">
...
что Roblox ещё разрешает, профили под каждую игру с фаст-флагами и обычными
настройками Roblox, выбор конкретного сервера перед заходом в игру, контракт
плагинов со своим SDK, значок в трее с фоновым режимом, уведомления о заходе
на сервер и интерфейс на русском и английском.
